...
# Do we want to initialize the partitions?
if [[ "${INIT}" == "1" ]]; then
	relfs_init /dev/ffx01
fi

# Check for Primary Kernel
if [[ -n "${KERNEL1}" ]]; then
	flash_kernel_image ${KERNEL1}
fi

# Check for Failsafe Kernel
if [[ -n "${KERNEL2}" ]]; then
	flash_mtd "fail-safe" ${KERNEL2}
fi

# Check for RootFS
if [[ -n "${ROOTFS}" ]]; then
	echo "Reflashing root file system"
	flash_relfs_part "/dev/ffx01" "1" "${ROOTFS}" "rootfs"
fi

# Reflash the IBCs
for _idx in ${REDUNDANT_IDX}; do

	if [[ -n "${IBC}" ]]; then
		echo
		echo "================================================================================"
		echo "Flashing IBC ${_idx}"
		echo "================================================================================"
		echo
		flash_mtd ibc ${IBC} ${_idx}
	fi


done

# Configure the JCI partition
if [[ -n "${JCI_IMG}" ]]; then
	flash_relfs_part "/dev/ffx01" "1" "${JCI_IMG}" "jci"
fi
